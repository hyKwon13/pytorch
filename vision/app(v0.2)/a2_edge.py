...
# 마우스 이벤트 처리 함수
def run_a2_edge(cap, label):
    def draw_rectangle(event, x, y, flags, param):
        nonlocal drawing, top_left_pt, bottom_right_pt, reference_contour_count
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            # 윤곽선 개수 저장
            reference_contour_count = len(contours)
    ...
    def process_and_display(frame, top_left_pt, bottom_right_pt):
        ...
        # 윤곽선 개수 비교 및 결과 출력
        result = compare_contour_count(reference_contour_count, len(contours))

        if not first_rectangle_drawn:
            # 첫 번째 사각형과 윤곽선을 frame0에 그립니다.
            cv2.rectangle(frame0, top_left_pt, bottom_right_pt, (0, 255, 0), 2)
            cv2.drawContours(frame0[top_left_pt[1]:bottom_right_pt[1], top_left_pt[0]:bottom_right_pt[0]], contours, -1, (0, 255, 0), 2)
            first_rectangle_drawn = True  # 첫 번째 사각형이 그려졌다고 표시
            
        if not result:
            # 윤곽선 개수가 일치하지 않을 때 빨간색으로 표시
    ...
    # 윤곽선 개수 비교 함수
    def compare_contour_count(count1, count2):
        # 윤곽선 개수 비교
        threshold = 2  # 적절한 임계값 설정
        return abs(count1 - count2) <= threshold
